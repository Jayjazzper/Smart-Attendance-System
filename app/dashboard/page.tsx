    ...
    <AdminGuard allowTeacher>
      ...
      {lockedClassroom ? (
        <div className="flex items-center gap-2">
          <span className="inline-flex items-center gap-1.5 rounded-xl bg-amber-50 border border-amber-200 px-4 py-2 text-xs font-bold text-amber-800 shadow-sm">
        ...
        <>
          ...
          <div className="grid gap-6 md:grid-cols-2 mt-2 animate-fade-in">
            ...
            <div className="rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
              <div className="flex items-center justify-between mb-6">
                <div>
                  <h3 className="text-base font-bold text-slate-900">ช่วงเวลาการเข้าเรียนวันนี้ (Peak Check-in Times)</h3>
                  <p className="text-[10px] font-semibold text-slate-500 mt-0.5">
          ...
          <div className="grid gap-6 md:grid-cols-12 mt-2 animate-fade-in">
            ...
            <div className="md:col-span-5 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm flex flex-col">
              <div className="mb-4">
                <h3 className="text-base font-bold text-slate-900">อันดับสถิติการเข้าเรียนรายห้อง (Leaderboard)</h3>
                <p className="text-[10px] font-semibold text-slate-500 mt-0.5">
              ...
              <div className="flex-1 flex flex-col gap-2.5 justify-center min-h-[300px]">
                {metrics.leaderboard.length > 0 ? (
                  metrics.leaderboard.slice(0, 6).map((item, index) => {
                    let rankBadge = "bg-slate-100 text-slate-500 border border-slate-200";
                    if (index === 0) rankBadge = "bg-amber-50 text-amber-600 border border-amber-100 font-black";
                    else if (index === 1) rankBadge = "bg-slate-100 text-slate-700 border border-slate-200 font-black";
                    else if (index === 2) rankBadge = "bg-orange-50 text-orange-600 border border-orange-100 font-black";

                    return (
                      <div key={item.classroom} className="flex items-center justify-between p-3 rounded-xl border border-slate-50 bg-slate-50/50 hover:bg-slate-50 transition-all">
                        <div className="flex items-center gap-3">
                          <span className={`inline-flex h-6 w-6 items-center justify-center rounded-lg text-xs shrink-0 ${rankBadge}`}>
                            {index + 1}
                          </span>
                          <div className="flex flex-col">
                            <span className="text-xs font-bold text-slate-900">ห้อง {item.classroom}</span>
                        ...
                        <div className="text-right">
                          <span className="text-xs font-extrabold text-slate-950 block">{item.rate}%</span>
                          <span className="text-[8px] font-semibold text-slate-400">ทั้งหมด {item.total} คน</span>
                        </div>
                      </div>
                    );
                  })
                ) : (
                  <div className="py-12 text-center text-xs font-bold text-slate-400 flex flex-col justify-center items-center gap-2">
                    <svg xmlns="http://www.w3.org/2000/svg" className="h-8 w-8 text-slate-300" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg>
            ...
            <div className="md:col-span-7 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm overflow-hidden flex flex-col justify-between">
              <div>
                <div className="flex items-center justify-between mb-4">
                  <div>
                    <h3 className="text-base font-bold text-slate-900">ประวัติการสแกนเช็คชื่อล่าสุด</h3>
                    <p className="text-[10px] font-semibold text-slate-500 mt-0.5">
                        ...
                        {metrics.recentScans.length > 0 ? (
                          metrics.recentScans.map((scan) => (
                            <tr key={scan.id} className="hover:bg-slate-50/50 transition-colors animate-fade-in">
                              <td className="py-3 px-2 text-slate-950 font-bold">{scan.time}</td>
                              <td className="py-3 px-2 font-bold text-slate-900">{scan.name}</td>
                              <td className="py-3 px-2">
                                {scan.classroom ? (
                                  <span className="rounded bg-blue-50 border border-blue-100 px-2 py-0.5 text-[9px] font-black text-blue-600">
                                    {scan.classroom}
                                  </span>
                                ) : (
