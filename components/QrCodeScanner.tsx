      ...
      try {
        ...
        await qrCode.start(
          { facingMode: "user" },
          {
            fps: 10,
            qrbox: (width, height) => {
              const boxWidth = Math.min(width, height) * 0.75;
              const boxHeight = Math.min(width, height) * 0.55;
        ...
        console.error("QR / Barcode start error:", err);
        setErrorText("ไม่สามารถเริ่มต้นกล้องสำหรับสแกน QR / Barcode ได้ กรุณาตรวจสอบสิทธิ์การเข้าถึงกล้อง");
      }
    }
    ...
    <div className="relative w-full h-full min-h-[300px] bg-slate-950 flex flex-col items-center justify-center">
      ...
      {!isScanning && !errorText && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 text-slate-400 z-30 bg-slate-950">
          ...
          <p className="text-xs font-bold text-slate-200">กำลังเตรียมกล้องสแกน QR / Barcode...</p>
        </div>
      )}
    </div>
  );
